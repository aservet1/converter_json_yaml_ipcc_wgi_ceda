...
def scan(text):
	...
	token_stream = []
	while len(text):
		token_stream.append(next_token(text))
...
def getFn(function):
	...
	if not function in fns.keys():
		print('invalid function specifier!!', function)
		exit(2)
...
def process(tok):
	...
	if tok.kind == 'plainText':
		...
		if '|' not in lex:
			...
			except KeyError: # make sure this trycatch isnt necessary, its just a stopper for a JSON key not existing right now
